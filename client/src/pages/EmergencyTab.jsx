...
export default function EmergencyTab() {
  ...
  const fetchHospitals = async location => {
    try {
      const BHILAI_DURG_BBOX = [81.10, 21.15, 81.50, 21.40];
      const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/hospital.json?proximity=${location.lng},${location.lat}&bbox=${BHILAI_DURG_BBOX.join(',')}&limit=10&access_token=${MAPBOX_TOKEN}`;
      const res = await fetch(url);
      const data = await res.json();

      if (data.features?.length > 0) {
        let parsed = data.features.map((d, i) => ({
          id: `HOSP_${i}`,
          name: d.text || 'City Hospital',
          location: { lat: d.center[1], lng: d.center[0] }
        }));
